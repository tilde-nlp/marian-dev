...
class DecoderState {
  private:
    ...
    std::vector<size_t> targetWords_;
  ...
  public:
    ...
    virtual void setTargetWords(const std::vector<size_t>& targetWords) {
      targetWords_ = targetWords;
    }

    virtual const std::vector<size_t>& getTargetWords() {
      return targetWords_;
    }
};
